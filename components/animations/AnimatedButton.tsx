...
async function AnimatedButton({ text, href }: IAnimatedButtonProps) {
  const t = await getTranslations();
  return (
    <Button
      size="lg"
      className="relative items-center overflow-hidden border border-secondary-900 bg-transparent hover:bg-transparent px-8 py-4 text-xs md:text-lg font-semibold group"
    >
      {/* Static background */}
      ...
      {/* Button content */}
      <Link href={href} className="relative z-10 text-secondary-900 transition-colors duration-500">
        {t(text)}
      </Link>
