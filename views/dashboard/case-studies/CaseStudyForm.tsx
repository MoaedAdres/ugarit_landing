...
export function CaseStudyForm({ formData, setFormData, onSubmit, isLoading = false, isEdit = false }: CaseStudyFormProps) {
	...
	console.log('CaseStudyForm received formData:', formData);
	console.log('setFormData function:', typeof setFormData);

	// Helper function to update form data
	const updateFormData = (updates: Partial<CaseStudyFormData>) => {
		setFormData({ ...formData, ...updates });
	};

	// Helper function to update locale data
	const updateLocaleData = (locale: "en" | "ar" | "fr", updates: Partial<CaseStudyFormData["en"]>) => {
		const newFormData = {
			...formData,
			[locale]: { ...formData[locale], ...updates },
		};
		console.log('Updating locale data:', { locale, updates, newFormData });
		setFormData(newFormData);
	};

	// Body block functions
	const addBodyBlock = (locale: "en" | "ar" | "fr") => {
		const value = tempInputs.bodyBlock[locale].trim();
		console.log(`Adding body block for ${locale}:`, value);
		if (value) {
			console.log(`Current body blocks for ${locale}:`, formData[locale].body_blocks);
			updateLocaleData(locale, {
				body_blocks: [...formData[locale].body_blocks, value],
			});
			setTempInputs((prev) => ({
				...prev,
				bodyBlock: { ...prev.bodyBlock, [locale]: "" },
			}));
			console.log("Body block added successfully");
		}
	};
	...
	const addKpi = (locale: "en" | "ar" | "fr") => {
		const key = tempInputs.kpiKey[locale].trim();
		const value = tempInputs.kpiValue[locale].trim();
		console.log(`Adding KPI for ${locale}:`, { key, value });
		if (key && value) {
			console.log(`Current KPIs for ${locale}:`, formData[locale].results_kpis);
			updateLocaleData(locale, {
				results_kpis: { ...formData[locale].results_kpis, [key]: value },
			});
			setTempInputs((prev) => ({
				...prev,
				kpiKey: { ...prev.kpiKey, [locale]: "" },
				kpiValue: { ...prev.kpiValue, [locale]: "" },
			}));
	...
	const removeKpi = (locale: "en" | "ar" | "fr", key: string) => {
		...
		delete newKpis[key];
		updateLocaleData(locale, { results_kpis: newKpis });
	};
	...
	const validateForm = (): boolean => {
		...
		if (!formData.testimonial_id || formData.testimonial_id <= 0) {
			nextErrors["testimonial_id"] = "Testimonial ID is required";
		}

		// Validate all translations
		const locales = ['en', 'ar', 'fr'] as const;
		const localeNames = { en: 'English', ar: 'Arabic', fr: 'French' };
		...
		locales.forEach(locale => {
			...
			if (!localeData.sector.trim()) {
				nextErrors[`${locale}.sector`] = `Sector is required (${localeName})`;
			}
			if (!localeData.problem.trim()) {
				nextErrors[`${locale}.problem`] = `Problem is required (${localeName})`;
			}
			if (!localeData.solution.trim()) {
				nextErrors[`${locale}.solution`] = `Solution is required (${localeName})`;
			}

			// Required body blocks
			if (localeData.body_blocks.length === 0) {
				nextErrors[`${locale}.body_blocks`] = `At least one body block is required (${localeName})`;
			}

			// Required KPIs
			if (Object.keys(localeData.results_kpis).length === 0) {
				nextErrors[`${locale}.results_kpis`] = `At least one KPI is required (${localeName})`;
			}
		});
		...
		if (Object.keys(nextErrors).length > 0) {
			...
			toast({
				...
				description: `Please fix ${errorCount} error(s). Check all language tabs for missing required fields.`,
				variant: "destructive",
			});
			return false;
		}
	...
	const convertToFormData = (data: CaseStudyFormData): FormData => {
		...
		locales.forEach((locale) => {
			...
			// Basic fields
			apiFormData.append(`${locale}[sector]`, localeData.sector);
			apiFormData.append(`${locale}[problem]`, localeData.problem);
			apiFormData.append(`${locale}[solution]`, localeData.solution);

			// Body blocks
			localeData.body_blocks.forEach((block, index) => {
				apiFormData.append(`${locale}[body_blocks][${index}]`, block);
			});

			// Results KPIs
			Object.entries(localeData.results_kpis).forEach(([key, value]) => {
				apiFormData.append(`${locale}[results_kpis][${key}]`, value);
			});
		});
	...
	const handleSubmit = () => {
		...
		console.log("Body blocks count by locale:", {
			en: formData.en.body_blocks.length,
			ar: formData.ar.body_blocks.length,
			fr: formData.fr.body_blocks.length,
		});
		console.log("KPIs count by locale:", {
			en: Object.keys(formData.en.results_kpis).length,
			ar: Object.keys(formData.ar.results_kpis).length,
			fr: Object.keys(formData.fr.results_kpis).length,
		});
	...
	const hasTabErrors = (locale: 'en' | 'ar' | 'fr') => {
		return Object.keys(errors).some(key => key.startsWith(`${locale}.`));
	};
				...
				contentComponent={
					<div className="space-y-6">
						<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
							...
							<div className="space-y-2">
								<Label htmlFor="testimonial_id" className="text-sm font-medium">
									Testimonial ID
								</Label>
								<Input
									id="testimonial_id"
									...
									onChange={(e) => updateFormData({ testimonial_id: parseInt(e.target.value) || 0 })}
									placeholder="1"
									className="h-11"
								/>
								{errors["testimonial_id"] && <p className="text-xs text-destructive">{errors["testimonial_id"]}</p>}
							</div>
						</div>
						...
						<div className="grid grid-cols-1 md:grid-cols-3 gap-6">
							<div className="space-y-2">
								...
								<RSelect
									...
									handleChange={(value: string) => updateFormData({ status: value as "draft" | "published" })}
				...
				contentComponent={
					<div className="space-y-6">
						...
						<div className="flex space-x-1 bg-muted p-1 rounded-lg">
							{tabs.map((tab) => (
								<button
									...
									} ${hasTabErrors(tab.key) ? "border-2 border-destructive/50" : ""}`}
								>
									<span>{tab.flag}</span>
									{tab.label}
									{hasTabErrors(tab.key) && (
						...
						<div className="space-y-6">
							...
							<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
								<div className="space-y-2">
									<Label htmlFor={`${activeTab}_sector`} className="text-sm font-medium">
										Sector
									</Label>
									<Input
										id={`${activeTab}_sector`}
										value={formData[activeTab].sector}
										onChange={(e) => updateLocaleData(activeTab, { sector: e.target.value })}
										placeholder={activeTab === "ar" ? "التعليم" : activeTab === "fr" ? "Éducation" : "Education"}
										className={`h-11 ${activeTab === "ar" ? "text-right" : ""}`}
										dir={activeTab === "ar" ? "rtl" : "ltr"}
									/>
									{errors[`${activeTab}.sector`] && <p className="text-xs text-destructive">{errors[`${activeTab}.sector`]}</p>}
								</div>
							</div>

							<div className="space-y-3">
								<Label htmlFor={`${activeTab}_problem`} className="text-sm font-medium">
									Problem
								</Label>
								<Textarea
									id={`${activeTab}_problem`}
									value={formData[activeTab].problem}
									onChange={(e) => updateLocaleData(activeTab, { problem: e.target.value })}
									placeholder={
										activeTab === "ar"
											? "صف التحديات الرئيسية التي واجهها العميل"
											: activeTab === "fr"
											? "Décrivez les principaux défis rencontrés par le client"
											: "Describe the main challenges faced by the client"
									}
									rows={4}
									className={`resize-none ${activeTab === "ar" ? "text-right" : ""}`}
									dir={activeTab === "ar" ? "rtl" : "ltr"}
								/>
								{errors[`${activeTab}.problem`] && <p className="text-xs text-destructive">{errors[`${activeTab}.problem`]}</p>}
							</div>

							<div className="space-y-3">
								<Label htmlFor={`${activeTab}_solution`} className="text-sm font-medium">
									Solution
								</Label>
								<Textarea
									id={`${activeTab}_solution`}
									value={formData[activeTab].solution}
									onChange={(e) => updateLocaleData(activeTab, { solution: e.target.value })}
									placeholder={
										activeTab === "ar"
											? "اشرح الحل الذي قدمته"
											: activeTab === "fr"
											? "Expliquez la solution que vous avez fournie"
									...
									dir={activeTab === "ar" ? "rtl" : "ltr"}
								/>
								{errors[`${activeTab}.solution`] && <p className="text-xs text-destructive">{errors[`${activeTab}.solution`]}</p>}
							</div>
							...
							<div className="space-y-4">
								<div className="flex items-center justify-between">
									...
									<Badge variant="secondary" className="text-xs">
										{formData[activeTab].body_blocks.length} items
									</Badge>
								</div>
								{errors[`${activeTab}.body_blocks`] && (
									<p className="text-xs text-destructive">{errors[`${activeTab}.body_blocks`]}</p>
								)}
								<RFlex className="gap-3">
									<Input
										value={tempInputs.bodyBlock[activeTab]}
										onChange={(e) =>
											setTempInputs((prev) => ({
												...prev,
												bodyBlock: { ...prev.bodyBlock, [activeTab]: e.target.value },
											}))
										}
										placeholder={
											activeTab === "ar" ? "أضف قسم محتوى" : activeTab === "fr" ? "Ajouter un bloc de contenu" : "Add a body block"
										}
							...
							<div className="space-y-4">
								<div className="flex items-center justify-between">
									...
									<Badge variant="secondary" className="text-xs">
										{Object.keys(formData[activeTab].results_kpis).length} items
									</Badge>
								</div>
								{errors[`${activeTab}.results_kpis`] && (
									<p className="text-xs text-destructive">{errors[`${activeTab}.results_kpis`]}</p>
								)}
								<div className="grid grid-cols-1 md:grid-cols-2 gap-3">
									<Input
										value={tempInputs.kpiKey[activeTab]}
										onChange={(e) =>
											setTempInputs((prev) => ({
												...prev,
												kpiKey: { ...prev.kpiKey, [activeTab]: e.target.value },
											}))
										}
										placeholder={
											activeTab === "ar" ? "مفتاح المؤشر" : activeTab === "fr" ? "Clé KPI" : "KPI Key (e.g., student_engagement)"
										}
										className="h-11"
									/>
									<Input
										value={tempInputs.kpiValue[activeTab]}
										onChange={(e) =>
											setTempInputs((prev) => ({
												...prev,
												kpiValue: { ...prev.kpiValue, [activeTab]: e.target.value },
											}))
										}
				...
				contentComponent={
					<div className="space-y-6">
						<div className="space-y-2">
							...
							<Input id="logo" type="file" accept="image/*" onChange={(e) => handleFileChange("logo", e.target.files)} className="h-11" />
						...
						<div className="space-y-2">
							...
							{formData.gallery && formData.gallery.length > 0 && (
								<p className="text-sm text-muted-foreground">Selected: {formData.gallery.length} file(s)</p>
							)}
						</div>
					</div>
				}
